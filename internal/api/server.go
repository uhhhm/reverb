...
func (s *Server) routes() {
	...
	s.router.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(pr chi.Router) {
			...
			pr.Get("/recommendations/shelves", s.handleShelves)
			pr.Get("/recommendations/mixes", s.handleMixes)
			pr.Get("/recommendations/mixes/{kind}", s.handleMix)
			pr.Post("/recommendations/mixes/{kind}/playlist", s.handleSaveMix)
			pr.Get("/recommendations/playlists/{id}/suggestions", s.handlePlaylistSuggestions)
