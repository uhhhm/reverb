...
- **Downloads and external playback**: yt-dlp runs in-process in an embedded
  ...
  packages (curl_cffi, Pillow) that have no iOS build. The phone starts with the
  bundled yt-dlp and can activate release-signed pure-Python updates after
  signature verification and a compatibility probe. Failed activation restores
  the previous imports; startup re-verifies installed packages and falls back
  to the previous signed package or the bundle.
