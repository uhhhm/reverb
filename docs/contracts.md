...
Run `make setup-contracts` once, then `make contracts` after editing the schemas. `make contracts-check` checks generated output without rewriting it. `make contracts-test` exercises Go HTTP handlers and validates their responses and serialized event payloads against the schemas, including the playlist, offline set and pairing responses the iOS app decodes. `make check` runs both.
...
- `ios/ReverbKit/Sources/ReverbAPI/openapi.yaml`: the operations the iOS app calls (`iosOperations` in `tools/contracts/generate.mjs`), each given an `operationId`, with every component they reference. swift-openapi-generator builds the Swift client from it when the app builds, so the check keeps the app's copy current rather than checking Swift output. Removing or renaming an operation the app uses fails generation.
